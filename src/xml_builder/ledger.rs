...
impl XmlBuilder {
    pub fn create_ledger_request(ledger_map: &serde_json::Map<String, Value>) -> Result<String> {
        ...
        if let Some(v) = ledger_map.get("OPENINGBALANCE") { s.push_str(&format!("<OPENINGBALANCE>{}</OPENINGBALANCE>\n", XmlBuilder::escape_text(v))); }
        ...
        if let Some(v) = ledger_map.get("LEDMAILINGDETAILS.LIST").and_then(|v| v.as_object()) {
            ...
            if let Some(addr_list) = v.get("ADDRESS.LIST").and_then(|x| x.as_array()) {
                ...
                for item in addr_list { if let Some(obj) = item.as_object() { XmlBuilder::append_simple_if(obj, "ADDRESS", &mut s); } }
        ...
        if let Some(v) = ledger_map.get("LEDGSTREGDETAILS.LIST").and_then(|v| v.as_object()) {
            s.push_str("<LEDGSTREGDETAILS.LIST>\n");
            for k in ["APPLICABLEFROM","GSTREGISTRATIONTYPE","GSTIN","PLACEOFSUPPLY","TRANSPORTERID","ISOTHTERRITORYASSESSEE","CONSIDERPURCHASEFOREXPORT","ISTRANSPORTER","ISCOMMONPARTY"] {
        ...
        if let Some(v) = ledger_map.get("PAYMENTDETAILS").and_then(|v| v.as_object()) {
            ...
            for k in ["PAYMENTFAVOURING","TRANSACTIONNAME","SETASDEFAULT","DEFAULTTRANSACTIONTYPE","CHEQUECROSSCOMMENT","VIRTUALPAYMENTADDRESS","IFSCODE","BANKNAME","ACCOUNTNUMBER"] {
                XmlBuilder::append_simple_if(v, k, &mut s);
            }
        ...
        if let Some(v) = ledger_map.get("TDSAPPLICABLE") { s.push_str(&format!("<TDSAPPLICABLE>{}</TDSAPPLICABLE>\n", XmlBuilder::escape_text(v))); }
        if let Some(v) = ledger_map.get("TDSCATEGORYDETAILS.LIST").and_then(|v| v.as_object()) {
        ...
        if let Some(v) = ledger_map.get("HSNDETAILS.LIST").and_then(|v| v.as_object()) {
        ...
        if let Some(v) = ledger_map.get("GSTDETAILS.LIST").and_then(|v| v.as_object()) {
            ...
            if let Some(state) = v.get("STATEWISEDETAILS.LIST").and_then(|x| x.as_object()) {
                ...
                if let Some(name) = state.get("STATENAME") {
                    if name.is_string() && name.as_str().unwrap_or("").starts_with("&#") { if let Some(sv) = name.as_str() { s.push_str(&format!("<STATENAME>{}</STATENAME>\n", sv)); } }
                    else { s.push_str(&format!("<STATENAME>{}</STATENAME>\n", XmlBuilder::escape_text(name))); }
                } else { s.push_str("<STATENAME>&#4; Any</STATENAME>\n"); }
        ...
        if let Some(Value::String(name)) = ledger_map.get("NAME").cloned() { s.push_str(&format!("<NAME>{}</NAME>\n", name)); }
        if let Some(Value::Array(alias_arr)) = ledger_map.get("ALIAS") { for alias in alias_arr { if let Value::String(a) = alias { s.push_str(&format!("<NAME>{}</NAME>\n", a)); } } }
