...
impl XmlBuilder {
    pub fn create_envelope(
        ...
        writer
            .write_event(Event::Start(BytesStart::new("ENVELOPE")))
        ...
        for (k, v) in header {
            if !v.is_null() {
                XmlBuilder::write_simple(&mut writer, k, v)?;
            }
        }
        writer
        ...
        if let Some(body_map) = body {
            ...
            if let Some(desc) = body_map.get("DESC").and_then(|v| v.as_object()) {
                ...
                if let Some(stat) = desc.get("STATICVARIABLES").and_then(|v| v.as_object()) {
                    writer
                        .write_event(Event::Start(BytesStart::new("STATICVARIABLES")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                    for (k, v) in stat {
                        XmlBuilder::write_simple(&mut writer, k, v)?;
                    }
                    writer
                        .write_event(Event::End(BytesEnd::new("STATICVARIABLES")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                }
                if let Some(tdl) = desc.get("TDL").and_then(|v| v.as_object()) {
                    writer
                        .write_event(Event::Start(BytesStart::new("TDL")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                    if let Some(tmsg) = tdl.get("TDLMESSAGE").and_then(|v| v.as_object()) {
                        writer
                            .write_event(Event::Start(BytesStart::new("TDLMESSAGE")))
                            .map_err(|e| TallyError::Xml(e.to_string()))?;
                        for (k, v) in tmsg {
                            XmlBuilder::write_kv_recursive(&mut writer, k, v)?;
                        }
                        writer
                            .write_event(Event::End(BytesEnd::new("TDLMESSAGE")))
                            .map_err(|e| TallyError::Xml(e.to_string()))?;
                    }
                    writer
                        .write_event(Event::End(BytesEnd::new("TDL")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                }
                if let Some(fetchlist) = desc.get("FETCHLIST").and_then(|v| v.as_object()) {
                    writer
                        .write_event(Event::Start(BytesStart::new("FETCHLIST")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                    for (k, v) in fetchlist {
                        XmlBuilder::write_kv_recursive(&mut writer, k, v)?;
                    }
                    writer
                        .write_event(Event::End(BytesEnd::new("FETCHLIST")))
                        .map_err(|e| TallyError::Xml(e.to_string()))?;
                }
                writer
