...
impl XmlBuilder {
    pub fn create_voucher_request(voucher_map: &serde_json::Map<String, Value>) -> Result<String> {
        ...
        w.write_event(Event::Start(BytesStart::new("ENVELOPE")))
        ...
        w.write_event(Event::Start(BytesStart::new("DATA")))
            .map_err(|e| TallyError::Xml(e.to_string()))?;
        w.write_event(Event::Start(BytesStart::new("TALLYMESSAGE")))
            .map_err(|e| TallyError::Xml(e.to_string()))?;
                ...
                Value::Array(arr) => {
                    for entry in arr {
                        if let Some(obj) = entry.as_object() {
                            w.write_event(Event::Start(BytesStart::new(k)))
                                .map_err(|e| TallyError::Xml(e.to_string()))?;
                            for (ek, ev) in obj {
                                XmlBuilder::write_simple(&mut w, ek, ev)?;
                            }
                            w.write_event(Event::End(BytesEnd::new(k)))
                                .map_err(|e| TallyError::Xml(e.to_string()))?;
                        }
                    }
                }
                _ => {
                    if k != "VOUCHERTYPENAME" {
                        XmlBuilder::write_simple(&mut w, k, v)?;
                    }
                }
            }
        }

        w.write_event(Event::End(BytesEnd::new("VOUCHER")))
            .map_err(|e| TallyError::Xml(e.to_string()))?;
        w.write_event(Event::End(BytesEnd::new("TALLYMESSAGE")))
            .map_err(|e| TallyError::Xml(e.to_string()))?;
