...
impl Group {
    pub fn validate(&self) -> Result<()> {
        if self.name.trim().is_empty() { return Err(TallyError::Validation("Group name is required".into())); }
    ...
    pub fn to_map(&self) -> serde_json::Map<String, serde_json::Value> {
        ...
        if let Some(v) = &self.group_type { m.insert("GROUP_TYPE".into(), json!(v)); }
        if let Some(v) = &self.alias { m.insert("ALIAS".into(), json!(v)); }
        if let Some(v) = &self.basic_group_is_calculable { m.insert("BASICGROUPISCALCULABLE".into(), json!(v)); }
        if let Some(v) = &self.is_addable { m.insert("ISADDABLE".into(), json!(v)); }
        if let Some(v) = &self.is_subledger { m.insert("ISSUBLEDGER".into(), json!(v)); }
        if let Some(v) = &self.addl_alloc_type { m.insert("ADDLALLOCTYPE".into(), json!(v)); }
        if let Some(v) = &self.as_original { m.insert("ASORIGINAL".into(), json!(v)); }
        if let Some(v) = &self.affects_gross_profit { m.insert("AFFECTSGROSSPROFIT".into(), json!(v)); }
        ...
        if self.gst_applicable_from.is_some() || self.gst_taxability.is_some() || self.gst_source_of_details.is_some() || self.gst_classification_name.is_some() || self.gst_state_name.is_some() || self.gst_rate_duty_head.is_some() || self.gst_rate_valuation_type.is_some() || self.gst_rate.is_some() {
            ...
            if self.gst_state_name.is_some() || self.gst_rate_duty_head.is_some() || self.gst_rate_valuation_type.is_some() || self.gst_rate.is_some() {
                ...
                if let Some(v) = &self.gst_state_name { state.insert("STATENAME".into(), json!(v)); } else { state.insert("STATENAME".into(), json!("&#4; Any")); }
