...
impl Ledger {
    pub fn validate(&self) -> Result<()> {
        if self.name.trim().is_empty() { return Err(TallyError::Validation("Ledger name is required".into())); }
        if let Some(v) = &self.is_billwise_on { if v != "Yes" && v != "No" { return Err(TallyError::Validation("is_billwise_on must be 'Yes' or 'No'".into())); } }
        if let Some(v) = &self.is_credit_days_chk_on { if v != "Yes" && v != "No" { return Err(TallyError::Validation("is_credit_days_chk_on must be 'Yes' or 'No'".into())); } }
        ...
        if let Some(v) = &self.tds_applicable { if v != "Applicable" && v != "Not Applicable" { return Err(TallyError::Validation("TDSAPPLICABLE must be 'Applicable' or 'Not Applicable'".into())); } }
        Ok(())
    }
    ...
    pub fn to_map(&self) -> serde_json::Map<String, serde_json::Value> {
        ...
        if let Some(v) = self.opening_balance { m.insert("OPENINGBALANCE".into(), json!(v)); }
        if let Some(v) = &self.income_tax_number { m.insert("INCOMETAXNUMBER".into(), json!(v)); }

        // top-level GST
        if let Some(v) = &self.gst_applicable { m.insert("GSTAPPLICABLE".into(), json!(v)); }
        if let Some(v) = &self.appropriate_for { m.insert("APPROPRIATEFOR".into(), json!(v)); }
        if let Some(v) = &self.gst_appropriate_to { m.insert("GSTAPPROPRIATETO".into(), json!(v)); }
        if let Some(v) = &self.excise_alloc_type { m.insert("EXCISEALLOCTYPE".into(), json!(v)); }
        if let Some(v) = &self.gst_type_of_supply { m.insert("GSTTYPEOFSUPPLY".into(), json!(v)); }
        if let Some(v) = &self.gst_duty_head { m.insert("GSTDUTYHEAD".into(), json!(v)); }
        if let Some(v) = self.rate_of_tax_calculation { m.insert("RATEOFTAXCALCULATION".into(), json!(format!(" {}", v))); }
        if let Some(v) = &self.tax_type { m.insert("TAXTYPE".into(), json!(v)); }

        if let Some(days) = self.bill_credit_period_days { m.insert("BILLCREDITPERIOD".into(), json!(format!("{} Days", days))); }
        if let Some(v) = &self.is_billwise_on { m.insert("ISBILLWISEON".into(), json!(v)); }
        if let Some(v) = &self.is_credit_days_chk_on { m.insert("ISCREDITDAYSCHKON".into(), json!(v)); }

        // mailing block
        if self.mailing_name.is_some() || self.mailing_address.is_some() || self.mailing_state.is_some() || self.mailing_country.is_some() || self.mailing_pincode.is_some() {
            let mut mailing = serde_json::Map::new();
            mailing.insert("APPLICABLEFROM".into(), json!("20250401"));
            mailing.insert("MAILINGNAME".into(), json!(self.mailing_name.clone().unwrap_or_else(|| self.name.clone())));
            ...
            mailing.insert("COUNTRY".into(), json!(self.mailing_country.clone().unwrap_or_else(|| "India".into())));
            if let Some(v) = &self.mailing_state { mailing.insert("STATE".into(), json!(v)); }
            if let Some(v) = &self.mailing_pincode { mailing.insert("PINCODE".into(), json!(v)); }
            m.insert("LEDMAILINGDETAILS.LIST".into(), Value::Object(mailing));
        }

        // Bank top-level fields
        if let Some(v) = &self.account_number { m.insert("BANKDETAILS".into(), json!(v)); }
        if let Some(v) = &self.ifsc_code { m.insert("IFSCODE".into(), json!(v)); }
        if let Some(v) = &self.bank_account_holder_name { m.insert("BANKACCHOLDERNAME".into(), json!(v)); }
        if let Some(v) = &self.swift_code { m.insert("SWIFTCODE".into(), json!(v)); }
        if let Some(v) = &self.branch_name { m.insert("BRANCHNAME".into(), json!(v)); }
        if let Some(v) = &self.bank_bsr_code { m.insert("BANKBSRCODE".into(), json!(v)); }
        if let Some(v) = self.od_limit { m.insert("ODLIMIT".into(), json!(v.to_string())); }

        // Payment details
        if self.default_transaction_type.is_some() || self.payment_favouring.is_some() || self.cheque_cross_comment.is_some() || self.virtual_payment_address.is_some() || self.beneficiary_code.is_some() || self.account_number.is_some() || self.ifsc_code.is_some() || self.bank_name.is_some() {
            let mut pay = serde_json::Map::new();
            pay.insert("PAYMENTFAVOURING".into(), json!(self.payment_favouring.clone().unwrap_or_else(|| self.name.clone())));
            pay.insert("TRANSACTIONNAME".into(), json!(self.transaction_name.clone().unwrap_or_else(|| "Primary".into())));
            pay.insert("SETASDEFAULT".into(), json!(self.set_as_default.clone().unwrap_or_else(|| "No".into())));
            if let Some(v) = &self.default_transaction_type { pay.insert("DEFAULTTRANSACTIONTYPE".into(), json!(v)); }
            if let Some(v) = &self.cheque_cross_comment { pay.insert("CHEQUECROSSCOMMENT".into(), json!(v)); }
            if let Some(v) = &self.virtual_payment_address { pay.insert("VIRTUALPAYMENTADDRESS".into(), json!(v)); }
            if let Some(v) = &self.ifsc_code { pay.insert("IFSCODE".into(), json!(v)); }
            if let Some(v) = &self.bank_name { pay.insert("BANKNAME".into(), json!(v)); }
            if let Some(v) = &self.account_number { pay.insert("ACCOUNTNUMBER".into(), json!(v)); }
            if let Some(code) = &self.beneficiary_code {
                let mut ben = serde_json::Map::new();
                ben.insert("BENEFICIARYCODE".into(), json!(code));
                pay.insert("BENEFICIARYCODEDETAILS".into(), serde_json::Value::Object(ben));
            }
            m.insert("PAYMENTDETAILS".into(), serde_json::Value::Object(pay));
        }

        // TDS block/flags
        if let Some(v) = &self.is_tds_applicable { m.insert("ISTDSAPPLICABLE".into(), json!(v)); }
        if let Some(v) = &self.tds_deductee_type { m.insert("TDSDEDUCTEETYPE".into(), json!(v)); }
        if let Some(v) = &self.deduct_tds_in_same_voucher { m.insert("DEDUCTINSAMEVCH".into(), json!(v)); }
        if let Some(v) = &self.tds_applicable { m.insert("TDSAPPLICABLE".into(), json!(v)); }
        if self.tds_category_name.is_some() {
            let mut tds = serde_json::Map::new();
            tds.insert("CATEGORYDATE".into(), json!(self.tds_category_date.clone().unwrap_or_else(|| "20250401".into())));
            tds.insert("CATEGORYNAME".into(), json!(self.tds_category_name.clone().unwrap()));
            m.insert("TDSCATEGORYDETAILS.LIST".into(), serde_json::Value::Object(tds));
        }
        ...
        // GST nested
        if self.gst_applicable_from.is_some() || self.gst_taxability.is_some() || self.gst_source_of_details.is_some() || self.gst_rate_duty_head.is_some() || self.gst_rate_valuation_type.is_some() || self.gst_rate.is_some() || self.gst_state_name.is_some() || self.gst_classification_name.is_some() {
            ...
            if self.gst_state_name.is_some() || self.gst_rate_duty_head.is_some() || self.gst_rate_valuation_type.is_some() || self.gst_rate.is_some() {
                ...
                if let Some(v) = &self.gst_state_name { state.insert("STATENAME".into(), json!(v)); }
